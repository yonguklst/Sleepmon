                                         ...
                                         ke_task_id_t const src_id)
{
    ...
    {
        platform_reset(RESET_AFTER_SPOTA_UPDATE);// APP_DISABLED state is used to wait the GAP_READY_EVT message
        ASSERT_ERR(0);
    }
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    {
        platform_reset(RESET_AFTER_SPOTA_UPDATE);// We are not in a Connected State
        ASSERT_ERR(0);
    }
                                           ...
                                           ke_task_id_t const src_id)
{
    if (ke_state_get(dest_id) == APP_DB_INIT)
    {
        ...
        {
            platform_reset(RESET_AFTER_SPOTA_UPDATE);// An error has occurred during database creation
            ASSERT_ERR(0);
        }
    }
    else
    {
        platform_reset(RESET_AFTER_SPOTA_UPDATE);// APP_DB_INIT state is used to wait the APP_MODULE_INIT_CMP_EVT message
        ASSERT_ERR(0);
    }
