...
/**
 ****************************************************************************************
 * @brief Configure Battery Service (no Security).
 *
...
void app_bass_enable(uint16_t conhdl)
{
    ...
    app_batt_config( (uint8_t)0, USE_BAT_LEVEL_ALERT, GPIO_BAT_LED_PORT, GPIO_BAT_LED_PIN);
...
{
	uint8_t batt_lvl;
	GPIO_ConfigurePin(GPIO_PORT_2, GPIO_PIN_6, OUTPUT, PID_GPIO, true);
    ...
    if(bat_lvl_alert_used && batt_lvl <5)
		...
		GPIO_ConfigurePin(GPIO_PORT_2, GPIO_PIN_6, OUTPUT, PID_GPIO, false);
